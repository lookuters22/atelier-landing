...
## 9. What Goes Where In The Database

### `photographers.settings`

Use for identity and onboarding metadata.
...
- currency
- admin mobile number
- onboarding version markers

This table is for studio setup, not for every preference in the business.

### `studio_business_profiles`

Use for what the studio offers.
...
This table helps Ana know whether an inquiry is even in scope before runtime policy matters.

### `playbook_rules`

Use for reusable operating policy.

Examples:

- `discount_quote = ask_first`
- `schedule_call = auto`
- `send_team_pii = ask_first`

This table is the reusable studio constitution.

### `memories`

Use for wedding-specific or person-specific truth.

Examples:

- this planner has a one-off exception
- this couple should never be auto-followed right now
- this wedding has a banking workaround already approved

This table is where case truth lives.

### `documents`

Use for actual business assets and restricted files.
...
This table is how the system references important files safely.

### `threads`

Use for the communication container.

Important ideas:

- which conversation this is
- which wedding it is attached to
- whether automation is allowed
- last inbound and outbound timestamps

This table is the operational shell around the conversation.

### `messages`

Use for actual inbound and outbound communication records.
...
This table is the ground truth of what was actually said or sent.

### `message_attachments`

Use for attachments tied to messages.
...
This table helps the system know that a message includes non-text material, even if the AI cannot safely judge the visual content itself.

### `drafts`

Use for approval-stage outbound content.
...
- decision mode
- source action key
- `locked_for_sending_at`
- `invalidated_at`

This table is the checkpoint between AI output and real outbound communication.

### `tasks`

Use for explicit operational follow-up.
...
- review a timeline later

This table is how the system remembers work without inventing ghost timers.

### `knowledge_base`

Use for reusable studio-wide knowledge.

Examples:

- approved brand voice guidance
- standard contract explanations
- standard publication wording
- reusable vendor-credit language

This table is for durable reusable knowledge, not for one-off exceptions or sensitive PII.

### `thread_summaries`

Use for compact session memory.

This helps keep prompts small by summarizing thread history instead of reloading everything every time.

### `escalation_requests`

Use for blocked cases where Ana asked the photographer for help.

Important ideas:

- what action was blocked
- why
- what was asked on WhatsApp
- whether the answer became a one-off or reusable rule

This table is the audit trail of uncertainty.

### `weddings`

Use for the main case record.
...
- stage
- package context
- pause and lock states such as `agency_cc_lock`

This table is the anchor record for the case.

### `wedding_people`

Use for linking people to a wedding with their role and authority.
...
This table is how the system understands who matters in what way.

## 10. Internal APIs And Worker Entry Points

These are the important internal surfaces that move data through the system.

### `webhook-web`

This is the inbound edge entry for web-originated messages or test submissions.

Current file:

- `supabase/functions/webhook-web/index.ts`

Its job is to receive input and emit an Inngest event into the backend workflow system.

### `webhook-whatsapp`

This is the inbound edge entry for WhatsApp.

Current file:

- `supabase/functions/webhook-whatsapp/index.ts`

In V3, this should become the photographer-to-Ana operator lane, not a general client lane.

### `webhook-approval`

This is the approval webhook surface.

Current file:

- `supabase/functions/webhook-approval/index.ts`

Its job is to emit approval events when a pending draft is approved.

### `api-resolve-draft`

This is the direct draft resolution endpoint.
...
- emit the correct event for outbound processing
- in V3, reject stale drafts before send

### `inngest`

This is the workflow event bus and worker host.
...
This is the layer that connects inbound events, orchestration, approvals, and outbound work.
