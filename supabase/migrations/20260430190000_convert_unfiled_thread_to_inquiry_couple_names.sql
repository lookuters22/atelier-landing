-- Optional couple + lead names from client (deterministic extraction); improved SQL fallback when omitted.
-- Replace single-uuid signature with (uuid, text, text) so PostgREST can pass optional names.

DROP FUNCTION IF EXISTS public.convert_unfiled_thread_to_inquiry(uuid);
...
COMMENT ON FUNCTION public.convert_unfiled_thread_to_inquiry(uuid, text, text) IS
  'Operator: create inquiry wedding + lead client from thread; optional couple/lead names from client (deterministic extraction).';

GRANT EXECUTE ON FUNCTION public.convert_unfiled_thread_to_inquiry(uuid, text, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.convert_unfiled_thread_to_inquiry(uuid, text, text) TO service_role;
