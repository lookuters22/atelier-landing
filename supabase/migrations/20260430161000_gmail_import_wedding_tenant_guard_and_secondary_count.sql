-- Tenant-safe wedding assignment for Gmail materialization RPCs (service_role cannot cross-file into another photographer's wedding).
-- Open backlog count RPC for repair ops dashboard.

-- ---------------------------------------------------------------------------
-- complete_gmail_import_materialize_new_thread: validate p_thread_wedding_id ∈ weddings for p_photographer_id
-- finalize_gmail_import_link_existing_thread: same before setting threads.wedding_id
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  IF p_thread_wedding_id IS NOT NULL THEN
    IF NOT EXISTS (
      ...
      WHERE w.id = p_thread_wedding_id
        AND w.photographer_id = p_photographer_id
    ) THEN
      RAISE EXCEPTION 'complete_gmail_import_materialize_new_thread: wedding not found or tenant mismatch';
    END IF;
  END IF;
...
COMMENT ON FUNCTION public.complete_gmail_import_materialize_new_thread IS
  'Service-role: atomically creates email thread + first message, links gmail_render_artifact when provided, approves import_candidate. When p_thread_wedding_id is set, it must exist on public.weddings for p_photographer_id.';
...
BEGIN
  ...
  IF p_thread_wedding_id IS NOT NULL THEN
    IF NOT EXISTS (
      ...
      RAISE EXCEPTION 'finalize_gmail_import_link_existing_thread: wedding not found or tenant mismatch';
    END IF;
...
COMMENT ON FUNCTION public.finalize_gmail_import_link_existing_thread IS
  'Service-role: files import_candidate to an existing thread; optionally sets threads.wedding_id for grouped imports. When p_thread_wedding_id is set, it must exist on public.weddings for p_photographer_id.';

-- ---------------------------------------------------------------------------
-- Backlog estimate for gmail-repair-ops (tenant-scoped via RPC param in app; this is global count for ops visibility — prefer app filter).
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.gmail_import_secondary_pending_open_count_for_photographer_v1(p_photographer_id uuid)
RETURNS bigint
LANGUAGE sql
STABLE
SECURITY INVOKER
...
AS $$
  SELECT count(*)::bigint
  FROM public.gmail_import_secondary_pending p
  WHERE p.status = 'open'
    AND p.photographer_id = p_photographer_id;
$$;

COMMENT ON FUNCTION public.gmail_import_secondary_pending_open_count_for_photographer_v1(uuid) IS
  'Count open gmail_import_secondary_pending rows for one photographer (repair ops).';

REVOKE ALL ON FUNCTION public.gmail_import_secondary_pending_open_count_for_photographer_v1(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.gmail_import_secondary_pending_open_count_for_photographer_v1(uuid) TO service_role;

-- Preserve explicit service_role grants after CREATE OR REPLACE.
