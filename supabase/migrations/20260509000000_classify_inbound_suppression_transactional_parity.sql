-- =============================================================================
-- Parity: SQL classify_inbound_suppression ↔ TypeScript inboundSuppressionClassifier
-- =============================================================================
-- Extends public.classify_inbound_suppression (3-arg: sender, subject, body — same
-- inputs as convert_unfiled_thread_to_inquiry) with:
--   - transactional_score + verdict transactional_non_client
--   - subject_transactional_receipt / body_transactional_receipt reasons
--   - subject promo tokens aligned with TS (special offer, webinar, % off, …)
--
-- Does NOT add optional headers: the convert RPC only passes sender/title/body.
-- Gmail import paths use the TS classifier with headers in Edge.
--
-- Precedence matches TS: marketing (>=3, wins ties) → system (>=3) → transactional (>=2)
-- → weak human.
-- =============================================================================
...
COMMENT ON FUNCTION public.classify_inbound_suppression(text, text, text) IS
  'Deterministic inbound suppression — mirrors src/lib/inboundSuppressionClassifier.ts for sender/subject/body (no optional RFC headers). Includes transactional_non_client for receipt/billing/order patterns. Used by convert_unfiled_thread_to_inquiry.';
