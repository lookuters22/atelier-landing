...
function LandingPage2Content() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      /* Hero meta strip — hidden on load, fades in on scroll.
         `fromTo` (not `from`) so the initial opacity:0 / y:24 state is
         applied immediately on mount and ScrollTrigger only releases the
         animation when the strip enters the viewport. */
      /* Hero meta strip — pre-hidden in CSS (opacity: 0).
         We wait for the user's *first* scroll event (any direction, any
         distance) before fading the cells in. This is what was originally
         requested: invisible on load, revealed as soon as scrolling
         begins — independent of viewport size or whether the strip
         happens to already be on screen. */
      const playHeroMeta = () => {
        gsap.to(".hero-meta > div", {
          y: 0,
          opacity: 1,
          duration: 0.7,
          ease: "power3.out",
        ...
        gsap.set(".hero-meta > div", { clearProps: "transform" });
        window.removeEventListener("scroll", playHeroMeta);
        window.removeEventListener("wheel", playHeroMeta);
        window.removeEventListener("touchmove", playHeroMeta);
        window.removeEventListener("keydown", playHeroMeta);
      };
      gsap.set(".hero-meta > div", { y: 24, opacity: 0 });
      window.addEventListener("scroll", playHeroMeta, { passive: true });
      window.addEventListener("wheel", playHeroMeta, { passive: true });
      window.addEventListener("touchmove", playHeroMeta, { passive: true });
      window.addEventListener("keydown", playHeroMeta);
